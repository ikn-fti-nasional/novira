...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2 items-start">
        ...
        <div class="space-y-4">
                            ...
                            <div class="max-h-56 space-y-0.5 overflow-y-auto">
                                {#each kameraTerfilter as kam (kam.id)}
                                    <button
                                        ...
                                        class="w-full rounded-sm px-2 py-1.5 text-left text-sm hover:bg-accent hover:text-accent-foreground {kam.id === kameraKotaDipilih ? 'bg-accent/60 font-medium' : ''}"
                                    >
                                        {kam.nama}
                                    </button>
                                {:else}
                        ...
                        <Card.Root class="overflow-hidden border-border/80 shadow-md">
                            <Card.Header class="flex flex-row items-center justify-between space-y-0 pb-3">
                                <div class="truncate pr-2">
                                    <Card.Title class="text-sm font-bold tracking-tight truncate">{kam.nama}</Card.Title>
            ...
            {:else}
                <div class="flex h-72 flex-col items-center justify-center gap-3 rounded-xl border border-dashed text-muted-foreground">
                    <VideoIcon class="size-10 stroke-1" />
                    <p class="text-sm text-center px-4">Pilih kota dan CCTV, lalu klik "Tampilkan" untuk memantau stream.</p>
                </div>
            {/if}
        </div>
