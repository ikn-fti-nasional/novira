...
<div class="space-y-6">
	<div class="flex flex-col gap-1 border-b border-border/60 pb-4">
		...
		<p class="text-sm text-muted-foreground">
			Umpan video RTSP real-time yang terhubung dengan mesin deteksi objek YOLOv8 dan ByteTrack persistence.
		</p>
	</div>
		...
		{#each data.kameraList as cam (cam.id)}
			<Card.Root class="overflow-hidden border-border/80 shadow-xs hover:shadow-md transition-shadow">
				<div class="relative aspect-video bg-slate-950">
					...
					{:else}
						<div class="flex h-full items-center justify-center text-slate-500 text-xs">OFFLINE / PERBAIKAN</div>
					{/if}
					<div class="absolute top-2 left-2 flex items-center gap-1.5">
						{#if cam.status === "ONLINE"}
							<span class="size-2.5 rounded-full bg-emerald-500 animate-pulse"></span>
							<span class="rounded bg-black/60 px-1.5 py-0.5 text-[9px] font-bold text-white">LANGSUNG</span>
						{:else}
							<span class="size-2.5 rounded-full bg-slate-500"></span>
							<span class="rounded bg-slate-800 px-1.5 py-0.5 text-[9px] font-bold text-slate-300">OFFLINE</span>
						{/if}
					</div>
				</div>
